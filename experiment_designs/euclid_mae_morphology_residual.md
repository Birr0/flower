...
1. **Design overview & architecture mapping** — base/condition/residual mapped onto Flower's
   `spender_*_flow` frozen-encoder + CFM machinery.
2. **Data** — Euclid Q1 sample, the aligned per-object triple (embedding / morphology / imaging),
   HF provenance, cross-match and sample definition.
3. **The base representation** — the Euclid galaxy MAE (384-d avg-pooled ViT); RR2-precomputed vs.
   DR1-embed-ourselves decision.
4. **The conditioning** — the GZ Euclid 13-question decision tree, tree-shaped missingness, and the
   `y_catalog` contract.
5. **The residual / anomaly score** — two scores (S1 absolute, S2 morphology-local), normalised MI,
   feature-set MIs via conditioning masks, and the artifact-conditioning ablation (A0/A1).
6. **Evaluation & validation** — expert inspection + four automated verification signals + positive
   controls + success criteria.
7. **Related work & novelty positioning** — Wu & Walmsley SAE paper and the claimed edge.
8. **Implementation plan** — reuse-first config/data work, GitHub dependencies, staging, tests,
   compute.
9. **Risks & open questions** — ranked risks and the consolidated `[TO VERIFY]` list.
10. **Timeline** — phased schedule, dependencies, and the critical path.
...
[`pmi_flux_mi_estimation.md`](./pmi_flux_mi_estimation.md). **This experiment is an application of
...
| **Dep A. Minimal pmi scorer** | Implement `log p(z\|y)` via probability-flow ODE + Hutchinson divergence and the S1/S2 + normalised-MI readouts, per `pmi_flux_mi_estimation.md`. Reusable across both designs. | — (parallel with 0) | **2–4 wk** |

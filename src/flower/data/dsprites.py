...
class dSprites(Dataset):
    ...
    def __getitem__(self, idx):
        # k: torch.as_tensor(data[k]).unsqueeze(0)
        ...
        return {
            ...
            "catalog": {
                "label_shape": torch.tensor(
                    data["label_shape"], dtype=torch.float32
                ).unsqueeze(0),
                "value_x_position": torch.tensor(pos_x, dtype=torch.float32).unsqueeze(
                    0
                ),
                "value_y_position": torch.tensor(pos_y, dtype=torch.float32).unsqueeze(
                    0
                ),
                "value_orientation_sin": torch.tensor(
                    orient_sin, dtype=torch.float32
                ).unsqueeze(0),
                "value_orientation_cos": torch.tensor(
                    orient_cos, dtype=torch.float32
                ).unsqueeze(0),
...
def prep_dsprites_subsample(n_train=100000, n_val=25000, n_test=25000, seed=42):
    ...
    val_test = train_tmp["test"].train_test_split(
        test_size=n_val / (n_val + n_test), seed=seed
    )
...
if __name__ == "__main__":
    ...
    for split_name in ["train", "val", "test"]:
        ...
        # Instantiate your dSprites class (Passing None for x_ds/y_catalog)
        ...
        # Before preprocessing: <PIL.PngImagePlugin.PngImageFile>
